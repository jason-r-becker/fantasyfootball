...
Copy [the placeholder Sleeper example](../examples/sleeper-config.example.json)
as a starting point.
...
| `draft_rounds` | integer | Recommended | Total rounds. `--rounds` overrides it; if both are absent the app uses 15. |
...
| `league_id` | string | Recommended for Sleeper | Lets the app fetch owner display names after pick sync. It is not needed to download picks. |
| `team_id` | integer | Recommended for ESPN | Identifies which ESPN team is yours. |
